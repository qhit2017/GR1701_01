...
--3、查全体学生的姓名及其出生年份。
SELECT year(GETDATE())-sage as 出生年份, Sname FROM tb_student
...
Select Sno,Sname ,ssex FROM tb_student where Sname not LIKE '张%'
--14.	查询姓“李”且全名为三个汉字的学生的姓名。  
Select Sno,Sname ,ssex FROM tb_student where Sname  LIKE '李__' 
...
--18.	查询每个专业内学生的平均成绩。
Select  sdept ,AVG(Grade)平均成绩 from tb_student GROUP BY Sdept
--19.	查所有有成绩的学生的学号和姓名。
...
--22.	查询男生和女生的人数。
SELECT COUNT(*)as 总人数,Ssex as 性别 from tb_student group BY Ssex
--23.	查询每个专业的平均成绩，结果按照成绩降序排列。
SELECT AVG(Grade) as 平均成绩,Sdept from tb_student group BY Sdept order by AVG(Grade) desc
--24.	查询每个专业中成绩最好的学生成绩。
SELECT max(Grade) as 最高分,Sdept from tb_student group BY Sdept
--25.	查询每个专业中成绩最差的学生成绩。
SELECT min(Grade) as 最低分,Sdept from tb_student group BY Sdept
--26.	查询平均成绩最差的专业的名字。
SELECT TOP 1 Sdept from tb_student group BY Sdept ORDER BY AVG(Grade)
--27.	查询男生和女生的平均成绩。
SELECT avg(Grade) as 平均成绩,Ssex as 性别 from tb_student group BY Ssex
